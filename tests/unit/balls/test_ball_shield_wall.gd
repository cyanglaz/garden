...
const BALL_DATA:BingoBallData = preload("res://data/balls/player/bingo_ball_shield_wall.tres")
...
func test_add_strength_to_spaces_on_the_same_column() -> void:
	...
	var script = partial_double(BingoBallScriptShieldWall).new(BALL_DATA)
	...
	for space in fixture_spaces:
		assert_eq(space.space_effect_manager.get_space_effect("shield").stack, 1)
	
	# Remove ball to remove strength from spaces
	script._handle_removed_from_board()
	for space in fixture_spaces:
		assert_null(space.space_effect_manager.get_space_effect("shield"))
