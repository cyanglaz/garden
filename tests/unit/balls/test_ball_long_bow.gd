...
const BALL_DATA:BingoBallData = preload("res://data/balls/player/bingo_ball_long_bow.tres")
...
func test_placement_rules() -> void:
	assert_eq(BALL_DATA.placement_rule, BingoBallData.PlacementRule.PRIORITIZE_LEFT)
...
func test_enhance_attack() -> void:
	...
	board.board[3].ball_data = BALL_DATA.get_duplicate()
