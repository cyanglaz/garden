...
func test_has_discard_hook_true_when_hand_empty() -> void:
	var t := _make_trinket()
	var cm := _make_combat_main_with_hand([])
	assert_true(t.has_discard_hook(cm, []))

func test_has_discard_hook_false_when_hand_has_cards() -> void:
	var t := _make_trinket()
...
func test_has_exhaust_hook_true_when_hand_empty() -> void:
	var t := _make_trinket()
	var cm := _make_combat_main_with_hand([])
	assert_true(t.has_exhaust_hook(cm, []))

func test_has_exhaust_hook_false_when_hand_has_cards() -> void:
	var t := _make_trinket()
	var cm := _make_combat_main_with_hand([ToolData.new()])
	assert_false(t.has_exhaust_hook(cm, []))
