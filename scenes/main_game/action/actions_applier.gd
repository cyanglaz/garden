...
func apply_actions(actions:Array, combat_main:CombatMain, tool_data:ToolData, secondary_card_datas:Array, tool_card:GUIToolCardButton) -> void:
	_pending_actions = actions.duplicate()
	_action_index = 0
	await _apply_next_action(combat_main, tool_data, secondary_card_datas, tool_card)

func _apply_next_action(combat_main:CombatMain, tool_data:ToolData, secondary_card_datas:Array, tool_card:GUIToolCardButton) -> void:
