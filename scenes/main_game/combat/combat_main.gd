...
signal lose(session_summary:SessionSummary, contract:ContractData)
...
var _gui:GUIMainGame: get = _get_gui
var _weak_gui:WeakRef = weakref(null)
...
var _rating:ResourcePoint

var _harvesting_fields:Array = []

func start(gui:GUIMainGame, field_count:int, card_pool:Array[ToolData], max_energy:int, contract:ContractData, rating:ResourcePoint) -> void:
	PopupThing.clear_popup_things()

	_weak_gui = weakref(gui)
	...
	_gui.bind_power_manager(power_manager)
	_gui.bind_energy(energy_tracker)
	_gui.bind_tool_deck(tool_manager.tool_deck)
	_gui.setup_plant_seed_animation_container(field_container)
	_gui.end_turn_button_pressed.connect(_on_end_turn_button_pressed)
	_gui.tool_selected.connect(_on_tool_selected)
	_gui.plant_seed_drawn_animation_completed.connect(_on_plant_seed_drawn_animation_completed)
	_gui.card_use_button_pressed.connect(_on_card_use_button_pressed)
	_gui.mouse_exited_card.connect(_on_mouse_exited_card)
...
func update_rating(val:int) -> void:
	_rating.value += val
	await _gui.rating_update_finished
	if _rating.value == 0:
		_lose()
...
func _start_new_level() -> void:
	...
	_gui.update_with_plants(plant_seed_manager.plant_datas)
...
func _lose() -> void:
	...
	await Util.create_scaled_timer(WIN_PAUSE_TIME).timeout
	_gui.toggle_all_ui(true)
	lose.emit(session_summary, _contract)
...
func _end_day() -> void:
	...
	await weather_manager.apply_weather_actions(field_container.fields, _gui.gui_weather_container.get_today_weather_icon())
	...
	await update_rating( -_contract.get_penalty_rate(day_manager.day))
	_start_day()
...
func _plant_new_seeds() -> void:
	...
	await plant_seed_manager.draw_plants(field_indices, _gui.gui_plant_seed_animation_container,)
...
func _harvest() -> bool:
	...
	await plant_seed_manager.finish_plants(field_indices_to_harvest, harvestable_plant_datas, _gui.gui_plant_seed_animation_container)
	if _met_win_condition():
		await _win()
		return true
	else:
		await plant_seed_manager.draw_plants(field_indices_to_harvest, _gui.gui_plant_seed_animation_container)
...
func _on_weathers_updated() -> void:
	_gui.update_weathers(weather_manager)
...
func _set_gold(val:int) -> void:
	_gold = val
	_gui.gui_shop_main.update_for_gold(_gold)

func _set_boost(val:int) -> void:
	boost = val
	_gui.update_boost(boost)
...
func _get_gui() -> GUIMainGame:
	return _weak_gui.get_ref()
