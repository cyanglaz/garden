...
func _ready() -> void:
	...
	_gui_main_game.plant_seed_selected.connect(_on_plant_seed_selected)
	...
	_gui_main_game.update_with_plant_datas(plant_seed_manager.plant_seeds)
	_gui_main_game.setup_tools(tool_manager.tools)
...
func _clear_plant_seed_selection() -> void:
	...
	_gui_main_game.toggle_following_plant_icon_visibility(false, null)
...
func _on_field_pressed(index:int) -> void:
	...
	if selected_plant_seed_data && !_field_container.is_field_occupied(index):
		...
		tool_manager.apply_field_tool(field)
...
func _on_plant_seed_selected(index:int) -> void:
	...
	if plant_data:
		_gui_main_game.toggle_following_plant_icon_visibility(true, plant_data)
		_clear_tool_selection()
	else:
		_clear_plant_seed_selection()
