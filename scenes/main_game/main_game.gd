...
func _on_tool_application_started(tool_data:ToolData) -> void:
	var card = gui_main_game.gui_tool_card_container.get_card(tool_manager.selected_tool_index)
	card.card_state = GUIToolCardButton.CardState.IN_USE
	_clear_tool_selection()
	gui_main_game.toggle_all_ui(false)
...
func _on_tool_application_completed(tool_data:ToolData) -> void:
	await _harvest()
	await tool_manager.discard_cards([tool_data], gui_main_game.gui_tool_card_container)
	gui_main_game.toggle_all_ui(true)
