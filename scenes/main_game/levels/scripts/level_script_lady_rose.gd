...
func _handle_level_start_hook(main_game:MainGame, icon:GUIEnemy) -> void:
	...
	var starting_position:Vector2 = icon.global_position + Vector2(0, icon.size.y * 2)
	for i in count:
		await main_game.tool_manager.add_tool_to_draw_pile(tool_data.get_duplicate(), starting_position, true, true)
