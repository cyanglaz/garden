class_name BingoBallScriptThrowingAxe
...
func _has_power_up(_bingo_result:BingoResult) -> bool:
	if !bingo_space_data:
		return false
	return !BingoBoard.is_corner(bingo_space_data.index) && BingoBoard.is_edge(bingo_space_data.index)

func evaluate_for_description() -> void:
	if _has_power_up(null):
		_bingo_ball_data.highlight_description_keys["dmg"] = true
		_bingo_ball_data.highlight_description_keys["edge"] = true
	else:
		_bingo_ball_data.highlight_description_keys["dmg"] = false
		_bingo_ball_data.highlight_description_keys["edge"] = false
