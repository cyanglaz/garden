class_name BingoBallScriptBattleFlag
...
func _handle_removed_from_board() -> void:
	for adjacent_space:BingoSpaceData in _get_spaces_on_the_same_column():
		adjacent_space.space_effect_manager.reduce_space_effect("strength", _bingo_ball_data.data["stack"] as int)

func _handle_placement_events() -> void:
	for adjacent_space:BingoSpaceData in _get_spaces_on_the_same_column():
		adjacent_space.space_effect_manager.add_space_effect(MainDatabase.space_effect_database.get_data_by_id("strength"), _bingo_ball_data.data["stack"] as int)
