...
func spawn_trinket(trinket_data: TrinketData, spawn_position: Vector2) -> void:
	...
	_original_trinket_size = gui_reward_trinket.size
...
func _animate_spawn(spawn_position: Vector2) -> void:
	...
	child.size = _original_trinket_size * INITIAL_SCALE_FACTOR
	child.show()
	await get_tree().process_frame
	var tween: Tween = Util.create_scaled_tween(self)
	var target_size := _original_trinket_size * DISPLAY_SCALE_FACTOR
	var target_position := Vector2(
		(size.x - target_size.x) / 2.0,
		(size.y - target_size.y) / 2.0
	)
	...
	tween.tween_property(child, "size", target_size, SPAWN_TRANSITION_TIME).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
...
func _on_trinket_reward_selected(trinket_data: TrinketData, gui_trinket: GUIChestRewardTrinket) -> void:
	...
	gui_trinket.queue_free()
	Events.request_add_trinket_to_collection.emit(trinket_data, from_global_position, DISPLAY_SCALE_FACTOR)
