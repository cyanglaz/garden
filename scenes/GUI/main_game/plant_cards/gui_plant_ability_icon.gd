...
func _on_mouse_entered() -> void:
	is_highlighted = true
	#var data:PlantAbilityData = MainDatabase.plant_ability_database.get_data_by_id(ability_id)
#	if !library_mode:
#		Singletons.main_game.hovered_data = data
#	if display_mode:
#		return
#	_weak_tooltip = weakref(Util.display_thing_data_tooltip(data, self, false, GUITooltip.TooltipPosition.RIGHT, true))
