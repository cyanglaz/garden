...
func update_gold(gold:int, animation_type:AnimationType, increment_step:int = 1) -> void:
		...
		if gold_diff < 0:
			...
			match animation_type:
				AnimationType.SINGLE:
					var delay := 0.0
					var i := 0
					while(_current_gold < gold):
						_current_gold += increment_step
						if _current_gold > gold:
							_current_gold = gold
							increment_step = gold - _current_gold
