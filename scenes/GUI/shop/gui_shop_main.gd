...
const TOOL_SHOP_PANEL_SCENE := preload("res://scenes/GUI/shop/shop_buttons/gui_tool_shop_panel.tscn")
const TRINKET_SHOP_PANEL_SCENE := preload("res://scenes/GUI/shop/shop_buttons/gui_trinket_shop_panel.tscn")
const ENCHANT_SHOP_PANEL_SCENE := preload("res://scenes/GUI/shop/shop_buttons/gui_enchant_shop_panel.tscn")
...
var _pending_enchant_panel: GUIEnchantShopPanel = null
...
func update_for_gold(gold:int) -> void:
	for gui_shop_panel: GUIShopPanel in tool_container.get_children():
		gui_shop_panel.update_for_gold(gold)
	for gui_shop_panel: GUIShopPanel in trinket_container.get_children():
		gui_shop_panel.update_for_gold(gold)
	for gui_shop_panel: GUIShopPanel in enchant_container.get_children():
		gui_shop_panel.update_for_gold(gold)
...
func _on_enchant_shop_button_pressed(gui_shop_panel: GUIEnchantShopPanel, enchant_data: EnchantData) -> void:
	...
	if !gui_shop_panel.sufficient_gold:
		...
		_insufficient_gold_audio.play()
		return
	_pending_enchant_panel = gui_shop_panel
...
func _on_enchant_card_pressed(tool_data: ToolData, enchant_card_global_position: Vector2) -> void:
	...
	if _pending_enchant_panel:
		_pending_enchant_panel.queue_free()
	_pending_enchant_panel = null
...
func _on_shop_panel_mouse_exited() -> void:
	Events.request_hide_warning.emit(WarningManager.WarningType.INSUFFICIENT_GOLD)
