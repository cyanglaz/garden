...
@onready var _gui_forge_confirmation_overlay: GUIForgeConfirmationOverlay = %GUIForgeConfirmationOverlay
...
func _ready() -> void:
	_gui_forge_confirmation_overlay.forge_confirmed.connect(_on_forge_confirmed)
	_gui_forge_confirmation_overlay.forge_canceled.connect(_on_forge_canceled)
...
func animate_show_with_balls(balls:Array[BingoBallData]) -> void:
	...
	for ball_data:BingoBallData in balls:
		...
		button.action_evoked.connect(_on_forge_ball_selected.bind(ball_data))
...
func _on_forge_ball_selected(ball_data:BingoBallData) -> void:
	_selected_ball_data = ball_data
	_gui_forge_confirmation_overlay.animate_show_with_ball_data(_selected_ball_data)
...
func _on_forge_confirmed(upgraded_bingo_ball_data:BingoBallData) -> void:
	upgraded_bingo_ball_data.owner = _selected_ball_data.owner
	_selected_ball_data.copy(upgraded_bingo_ball_data)
	_gui_forge_confirmation_overlay.hide()
	animate_hide()
	forge_completed.emit()

func _on_forge_canceled() -> void:
	_gui_forge_confirmation_overlay.hide()
