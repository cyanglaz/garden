...
func update(actions:Array[ActionData], target_field:Field) -> void:
	...
	for action_data:ActionData in actions:
		if action_data.value_type == ActionData.ValueType.X:
			...
			action_scene_x.update_for_x(action_data.get_calculated_x_value(target_field), action_data.x_value_type)
		var action_scene:GUIGeneralAction = GUI_GENERAL_ACTION_SCENE.instantiate()
		_action_container.add_child(action_scene)
		action_scene.update_with_action(action_data, target_field)
