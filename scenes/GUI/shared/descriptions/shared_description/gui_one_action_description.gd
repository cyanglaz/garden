...
func update_with_action_data(action_data:ActionData, target_plant:Plant) -> void:
	var resource_id := Util.get_action_id_with_action_type(action_data.type)
	texture_rect.texture = load(Util.get_image_path_for_resource_id(resource_id))
	title_label.text = _get_action_name(action_data)
	rich_text_label.text = _get_action_description(action_data, target_plant)
...
func _get_action_description(action_data:ActionData, target_plant:Plant) -> String:
	var action_description := ActionDescriptionFormulator.get_action_description(action_data, target_plant)
