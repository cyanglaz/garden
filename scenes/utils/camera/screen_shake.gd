...
const SHAKE_AMPLITUDE := Vector2(10, 10)
const NOISE_SPEED := 1
...
func _shake():
	...
	_camera.offset.x = _amplitude.x * amount * _noise.get_noise_2d(_noise.seed, _noise_y)
	_camera.offset.y = _amplitude.y * amount * _noise.get_noise_2d(_noise.seed*2, _noise_y)
	_camera.rotation = _rotation * amount * _noise.get_noise_2d(_noise.seed*3, _noise_y)

#func _input(event):
	#if event.is_action_pressed("toggle_weapon"):
		#start(0.8, Vector2(30, 10), 0.0, 0.8, 0)
